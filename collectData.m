function DATASET = collectData(D,N,xmin,xmax,umin,umax,connected,plts)
    ...
    % place each segment into its dataset
        ...
        for d = 1:D
            ...
            [t,x] = ode45(@(t,x) em_CSTR(t,x,u(1,:,d)), T, x0(:,d));
            ...
            for n = 2:N+1
                ...
                if plts 
                    ...
                    plot(xs,'ro','markersize',15,'linewidth',2)
                    axis(1*[xmin(1) xmax(1) xmin(2) xmax(2)])
                    title(['Data collection for ' num2str(n-1) ' control moves!'])
                end

                DATASET{1,n-1}(d,:) = [x_steps(1,:) u(1,1:n-1,d) x_steps(n,1)];
                DATASET{2,n-1}(d,:) = [x_steps(1,:) u(1,1:n-1,d) x_steps(n,2)];
            end
        end
     ...
     % Fixes the horizon, build entire dataset for it, increases horizon
        ...
        for n = 1:N
            ...
            for d = 1:D(n)
                ...
                [t,x] = ode45(@(t,x) em_CSTR(t,x,u), T, x0);
                ...
                %DATASET_x1{n}(d,:) = [x_steps(1,:) u x_steps(end,1)];
                %DATASET_x2{n}(d,:) = [x_steps(1,:) u x_steps(end,2)];
                DATASET{1,n}(d,:) = [x_steps(1,:) u x_steps(end,1)];
                DATASET{2,n}(d,:) = [x_steps(1,:) u x_steps(end,2)];
            end
